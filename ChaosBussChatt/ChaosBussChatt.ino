/*
 ...
 ¨to chaos by De Dalsmo
   Notable contributions by loka and Boda
...
void loop(void)
{
  ...
  while(Serial.available()){
    ...
    delay(20); // I dont know why this is neade but it is neaded.
    SendMess.concat(character);
    hasMessageToSend = true;  
  }
  
  // if we have a message to send we send it
  if (hasMessageToSend){   
    radio.openWritingPipe(pipes[0]);  radio.stopListening(); // Stop listening and open pipes, so we can talk. 
